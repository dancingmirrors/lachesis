...
static int opt_show_mode(void *optctx av_unused, const char *opt, const char *arg) {
...
static const OptionDef options[] = {
    ...
    {"ast", OPT_TYPE_STRING, OPT_EXPERT, {&wanted_stream_spec[AVMEDIA_TYPE_AUDIO]}, "select the desired audio stream", "stream_specifier"},
    ...
    {"t", OPT_TYPE_TIME, 0, {&duration}, "play this duration of seconds of audio/video", "duration"},
    ...
    {"fast", OPT_TYPE_BOOL, OPT_EXPERT, {&fast}, "enable non-compliant optimizations", ""},
    {"genpts", OPT_TYPE_BOOL, OPT_EXPERT, {&genpts}, "generate PTS", ""},
    {"drp", OPT_TYPE_INT, OPT_EXPERT, {&decoder_reorder_pts}, "let the decoder reorder PTS (0 = off, 1 = on, -1 = auto)", ""},
    {"sync", OPT_TYPE_FUNC, OPT_FUNC_ARG | OPT_EXPERT, {.func_arg = opt_sync}, "set the audio-video sync type (audio, video, ext)", "type"},
    {"skip-to-keyframe", OPT_TYPE_BOOL, OPT_EXPERT, {&skip_to_keyframe}, "skip the video forward to keyframes instead of slowing down (drops content)", ""},
    {"no-shader-cache", OPT_TYPE_BOOL, OPT_EXPERT, {&no_shader_cache}, "disable caching compiled shaders on disk", ""},
    {"shader-cache-dir", OPT_TYPE_STRING, OPT_EXPERT, {&shader_cache_dir}, "directory for the shader cache", "dir"},
    {"keep-open", OPT_TYPE_BOOL, OPT_EXPERT, {&keep_open}, "keep the window open at the end of the first input", ""},
    ...
    {"loop", OPT_TYPE_INT, OPT_EXPERT, {&loop}, "set the number of times playback will be looped", "loop count"},
    {"framedrop", OPT_TYPE_BOOL, OPT_EXPERT, {&framedrop}, "drop frames when the CPU is too slow", ""},
    {"infbuf", OPT_TYPE_BOOL, OPT_EXPERT, {&infinite_buffer}, "don't limit the input buffer size (useful with realtime streams)", ""},
    {"cache-secs", OPT_TYPE_FLOAT, OPT_EXPERT, {&opt_cache_secs}, "stream readahead in seconds (-1 = auto: 30 for network, 1 for local)", "seconds"},
    {"cache-size", OPT_TYPE_INT, OPT_EXPERT, {&opt_cache_size_mb}, "max readahead buffer in MB (-1 = auto: 128 for network, 15 for local)", "MB"},
    ...
    {"acodec", OPT_TYPE_STRING, OPT_EXPERT, {&audio_codec_name}, "force an audio decoder", "decoder_name"},
    {"scodec", OPT_TYPE_STRING, OPT_EXPERT, {&subtitle_codec_name}, "force a subtitle decoder", "decoder_name"},
    {"vcodec", OPT_TYPE_STRING, OPT_EXPERT, {&video_codec_name}, "force a video decoder", "decoder_name"},
    {"autorotate", OPT_TYPE_BOOL, 0, {&autorotate}, "automatically rotate video", ""},
    {"find_stream_info", OPT_TYPE_BOOL, OPT_INPUT | OPT_EXPERT, {&find_stream_info}, "read and decode the stream(s) to fill missing information with heuristics"},
    ...
    {"vulkan-swap-mode", OPT_TYPE_STRING, OPT_EXPERT, {&vulkan_swap_mode}, "Vulkan present mode (fifo, fifo-relaxed, mailbox, immediate)", "mode"},
    {"video_bg", OPT_TYPE_STRING, OPT_EXPERT, {&video_background}, "set the video background for transparent content"},
    ...
    {"360-sbs", OPT_TYPE_BOOL, 0, {&enable_360sbs}, "enable 360\xc2\xb0 equirectangular projection"},
    ...
    {"no-ytdl-chunked", OPT_TYPE_BOOL, OPT_EXPERT, {&ytdl_chunked_disable}, "disable yt-dlp's chunked resumable HTTP"},
    {
        NULL,
    },
};
...
static int opt_version(void *optctx av_unused, const char *opt av_unused,

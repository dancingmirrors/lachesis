...
void deinterlace_prepare(VideoState *is, Frame *vp) {
    ...
    is->render_params.prev_frame_id = 0;
    is->render_params.next_frame_id = 0;
    ...
    if (is->deint_prev && is->deint_prev->width > 0 &&
        ...
        is->render_params.prev_frame_id = is->deint_prev_id;
    }
    if (frame_queue_nb_remaining(&is->pictq) > 0) {
        ...
        if (nextvp != vp && nextvp->serial == vp->serial) {
            ...
            is->render_params.next_frame_id = nextvp->id;
        }
    }
}

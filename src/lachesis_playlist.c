...
static int playlist_same_archive(int a, int b) {
    const PlaylistEntry *ea = playlist_get(a);
    const PlaylistEntry *eb = playlist_get(b);

    if (!ea || !eb || !ea->archive_path || !eb->archive_path) {
        return 0;
    }
...
static int playlist_archive_start(int pos) {
    if (pos < 0 || pos >= playlist_size) {
        return -1;
    }
    while (pos > 0 && playlist_same_archive(pos - 1, pos)) {
        pos--;
    }
...
int playlist_archive_next(int pos) {
    if (pos < 0 || pos >= playlist_size) {
        return -1;
    }
    for (int i = pos + 1; i < playlist_size; i++) {
        if (!playlist_same_archive(i - 1, i)) {
            return i;
        }
    }
...
int playlist_archive_prev(int pos, int land_on_last) {
    int start = playlist_archive_start(pos);
    ...
    return playlist_archive_start(start - 1);
}
...
void playlist_remove_range(int pos, int count) {
    if (pos < 0 || count <= 0 || pos >= playlist_size) {
        return;
    }
    if (count > playlist_size - pos) {
        count = playlist_size - pos;
    }

    for (int i = pos; i < pos + count; i++) {
        PlaylistEntry *e = &playlist_entries[i];

        av_free(e->display_path);
        av_free(e->archive_path);
        av_free(e->entry_name);
    }
    memmove(&playlist_entries[pos], &playlist_entries[pos + count],
            (playlist_size - pos - count) * sizeof(*playlist_entries));
    playlist_size -= count;

    if (playlist_pos >= pos + count) {
        playlist_pos -= count;
    } else if (playlist_pos > pos) {
        playlist_pos = pos;
    }
}
...
void playlist_drop_unplayable(int from, int format_forced) {
    if (from < 0) {
        from = 0;
    }

    for (int i = playlist_size - 1; i >= from; i--) {
        const PlaylistEntry *e = &playlist_entries[i];
        const char *path = e->archive_path ? e->archive_path : e->display_path;
        const char *proto;
        struct stat st;

        if (!playlist_entry_is_reachable(i)) {
            playlist_remove_at(i);
            continue;
        }
        if (format_forced || !path) {
            continue;
        }
        if (!(proto = avio_find_protocol_name(path))) {
            continue;
        }
        if (strcmp(proto, "file")) {
            if (entry_is_inherited_terminal(path, proto)) {
...
static const char *const media_ext =
    ...
    "trp,ts,vid,vob,vro,weba,wmv,wtv,"
...
void playlist_warn_unsafe_disabled(const char *path, int open_failed) {
    ...
    warned = 1;
    log_warn("%s: playlists are only expanded with -%s.\n", path,
             option_name(options, &allow_unsafe));
}
...
void playlist_add_directory(const char *dir_path) {
    ...
    if (n > 1) {
        qsort(names, n, sizeof(*names), natural_name_cmp);
    }

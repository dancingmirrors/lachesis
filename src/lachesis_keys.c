...
static void show_zoom(float zoom) {
    if (zoom <= 0.0f) {
        return;
    }
    osd_show_message("Zoom: %.0f%%", (double)(zoom * 100.0f));
}
...
static void playlist_step_entry(VideoState **pis, int dir) {
    playlist_nav_dir = dir;
    if (dir > 0) {
        if (playlist_pos + 1 < playlist_size) {
            playlist_switch(pis, playlist_pos + 1);
        } else if (!keep_open) {
    ...
    } else if (playlist_pos > 0) {
        playlist_switch(pis, playlist_pos - 1);
    }
}

static void playlist_step_archive(VideoState **pis, int dir) {
    int target = dir > 0
        ? playlist_archive_next(playlist_pos)
        : playlist_archive_prev(playlist_pos, archive_jump_last);

    if (target >= 0) {
        playlist_nav_dir = dir;
        playlist_switch(pis, target);
        return;
    }
    if (dir > 0 ? playlist_pos + 1 >= playlist_size : playlist_pos <= 0) {
        playlist_step_entry(pis, dir);
        return;
    }
    osd_show_message(dir > 0 ? "No next archive" : "No previous archive");
    (*pis)->force_refresh = 1;
}
    ...
    for (;;) {
        ...
        case SDL_EVENT_KEY_DOWN:
            ...
            if (allow_delete && event.key.key == SDLK_DELETE &&
                ...
                if (reason) {
                    ...
                    delete_prearm_paused = pause_to_carry(cur_stream);
                    delete_paused_by_prompt = 0;
            ...
            if ((event.key.mod & SDL_KMOD_ALT) && (event.key.mod & SDL_KMOD_SHIFT)) {
                ...
                if (sc == SDL_SCANCODE_EQUALS || sc == SDL_SCANCODE_MINUS) {
                    show_zoom(view_zoom_step(cur_stream,
                                             sc == SDL_SCANCODE_EQUALS ? 1 : -1));
                    cur_stream->force_refresh = 1;
                    break;
                    ...
                    show_zoom(view_zoom_reset(cur_stream));
                    cur_stream->force_refresh = 1;
                    break;
                }
            }
            ...
            case SDLK_GREATER:
                if (event.key.mod & SDL_KMOD_SHIFT) {
                    playlist_step_entry(pis, 1);
                } else {
                    playlist_step_archive(pis, 1);
                }
                break;
            case SDLK_COMMA:
            case SDLK_LESS:
        ...
        case SDL_EVENT_MOUSE_MOTION:
            ...
            if (pan_drag && event.type == SDL_EVENT_MOUSE_MOTION) {
                ...
                if (dx || dy) {
                    view_pan_by(cur_stream, (float)dx, (float)dy);
                    cur_stream->force_refresh = 1;
                }
                break;
            }
        ...
        case SDL_EVENT_WINDOW_RESIZED:
            note_window_resized(cur_stream, event.window.data1,
                                event.window.data2);
            break;
        ...
        case SDL_EVENT_WINDOW_LEAVE_FULLSCREEN:
            note_fullscreen_state(cur_stream);
            break;
        case SDL_EVENT_WINDOW_ICCPROF_CHANGED:
        case SDL_EVENT_WINDOW_HDR_STATE_CHANGED:
            note_display_info_change(cur_stream);
            break;
        ...
        case FF_SCREENSHOT_EVENT:
            ...
            break;
        case FF_SINGLE_EVENT:
            single_handle_event(pis);
            break;
        default:
            break;
        }

 ...
 * Copyright © 2000-2026 Fabrice Bellard, and the FFmpeg authors.
 */
...
#define OPT_EXIT (1 << 1)
#define OPT_VIDEO (1 << 2)
#define OPT_AUDIO (1 << 3)
#define OPT_SUBTITLE (1 << 4)
#define OPT_DATA (1 << 5)
#define OPT_PERFILE (1 << 6)
#define OPT_FLAG_OFFSET (1 << 7)
#define OPT_OFFSET (OPT_FLAG_OFFSET | OPT_PERFILE)
#define OPT_FLAG_SPEC (1 << 8)
#define OPT_SPEC (OPT_FLAG_SPEC | OPT_OFFSET)
#define OPT_FLAG_PERSTREAM (1 << 9)
...
#ifdef _WIN32
void win32_argv_to_utf8(int *argc, char ***argv);
#endif
...
int parse_config_option(void *optctx, const char *opt, const char *arg,
                        const OptionDef *options, const char *src);
void parse_loglevel(int argc, char **argv, const OptionDef *options);
void parse_quiet(int argc, char **argv, const OptionDef *options);

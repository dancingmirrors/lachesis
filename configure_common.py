...
def check(
    ...
    # required=True derives a generic message from the check itself.
    # Otherwise you can pass a string if need be.
    if required is True:
        assert desc, "required=True needs a name or desc to derive a message from"
        if fn:
            required = "%s: missing, broken, or too old" % desc
        else:
            required = "%s: required dependency not found" % desc
...
def check_cc(
    ...
    args = get_program("CC").split() + [source] + _G.cflags + flags + _G.final_cflags
    if use_linking:
...
def check_cflags(*flags, final=False):
    keep_cflags, keep_final = len(_G.cflags), len(_G.final_cflags)
    if final:
        add_final_cflags(*flags)
        ok = check_cc(flags=["-Werror"])
    else:
        ok = check_cc(flags=["-Werror"] + list(flags))
    del _G.cflags[keep_cflags:]
    if not ok:
        del _G.final_cflags[keep_final:]
        return False
    if not final:
        add_cflags(*flags)
    return True
